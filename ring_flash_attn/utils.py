...
class All2AllComm:
    ...
    def all2all(self, to_send: torch.Tensor) -> None:
        self.res = [torch.empty_like(to_send) for _ in range(self.world_size)]

        input_tensor_list = [to_send] * self.world_size

        req = dist.all_to_all(output_tensor_list=self.res, input_tensor_list=input_tensor_list, async_op=True)
        self._reqs = [req]
    ...
    def data(self):
        if self._reqs is not None:
            raise RuntimeError("get data before communication finished")
        return self.res
